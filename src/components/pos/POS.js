...
import { chunk, Warning, f, getClientY, getClientX, isDrink, Random, keys } from '../../helpers/utils';
...
import { nonKitchenItems } from '../../objects/meta.js';
import NumPad from './NumPad.js';

export const CalcButton = ({ onClick = () => { }, disabled, text, style }) => {
...
export const getSessionData = (arr,total) => {
    return arr.reduce(
        (acc, { stock, id, price, ...rest }) => {
            acc.taste[id] = acc.taste[id] || (rest.taste ?? "");
            acc.note[id] = acc.note[id] || (rest.note ?? "");
        ...
        { taste: {}, total: 0, quantity: {}, price: {}, note: {} }
    )
}
function POS() {
    ...
    const [order, setOrder] = useState(tableOrders[activeSession ?? ""] ?
        { ...tableOrders[activeSession ?? ""], products: keys(tableOrders[activeSession ?? ""].data?.quantity ?? {}).map(id => Number(id)) }
        : {}
    );
    const placeOrder = async () => {
        ...
        setOrder(() => ({
            ...
            products: keys(JSON.parse(data.order.data).quantity).map(pr => Number(pr))
        }));
    ...
    const addToCart = (prID, cat = null) => {
        ...
        const isdrink = isDrink(cat);
        if (!isdrink && !itemChosen) return setItemChosen(product);
        ...
        if (isdrink) toast.success("Drink ka item");
        // check if the product is already in cart;
        // new check added it should not be vegetable
        if (thisProduct && !split) { // && !included.test(cat.trim())

            let qt, updatedStock, availableStock = thisProduct.quantity;
            if (itemChosen && isdrink) {
                qt = thisProduct.quantity;
                qt = qt < 1 ? qt : itemChosen.stock - 0;
                updatedStock = (thisProduct.stock - 0) + qt;
                availableStock = product.quantity - updatedStock;
            } else {
                qt = (thisProduct.stock - 0) + itemChosen.stock;
                updatedStock = qt;
            }
            console.log("Product already in cart, updating quantity...", qt, availableStock, updatedStock);
            ...
            if (inventory && sale && isdrink) setAvailableStocks({ ...availableStocks, [product.id]: availableStock });
            thisProduct.stock = updatedStock;
            thisProduct.taste = itemChosen?.taste ?? '';
            thisProduct.note = itemChosen?.note ?? '';
            ...
            window.electronAPI?.reloadWindow({
                total: f(getTotal(copyKartProducts) - (paidStat[table] || 0)),
                table: activeSession
            });
            ...
            product = {
                ...product,
                taste: itemChosen?.taste ?? '',
                note: itemChosen?.note ?? '',
                stock: itemChosen?.stock ?? 1
            }  // adds a single item on each click
            ...
            if (inventory && sale && isdrink && availableStock === -1) { // when the quantity was 0 there was 1 added as extra
                return document.querySelector('.also[data-id="' + product.id + '"]').classList.add('stock-out');
            }
            ...
            window.electronAPI?.reloadWindow({
                total: f(getTotal(rest) - (paidStat[table] || 0)),
                table: activeSession
            });
        }
    ...
    const cartClass = (item, index) => {
        ...
        if (currentProduct === index) {
            if (order.products ?? [].indexOf(item.id) !== -1) {
                if (order.data.quantity && order.data?.quantity[item.id] !== item.stock) {
                    className += "selected";
                }
            } else {
                className += "selected";
    ...
    const changeInput = (input, e) => {
        ...
        if (editing) { // price
            ...
            let Kart = {
                ...KartProducts, [activeSession]: KartProducts[activeSession].map((item, i) => {
                    if (i === currentProduct) {
    ...
    useEffect(() => {
        ...
        if (keys(openingCash).length === 0) {
            axios.get(`/pos/last-active-session`, {
                headers: {
    ...
    const noCash = keys(openingAmountSet).length === 0 || !openingAmountSet.status === true;
    return (
        <>
            <div className={`col-md-12 ${type === 'customer' ? "d-flex align-self-center position-relative" : "d-flex"}`} >
                {noCash && type !== 'customer' ? (
                        ...
                        (
                            <div ref={sectionRef} className={`container ms-2 put-here ${KartProducts[activeSession] && KartProducts[activeSession].length && type !== 'customer' ? 'action-visible' : ''} ${minned ? "fully-visible" : ""}`} style={{ borderRadius: '20px', backgroundColor: '#dadada' }}>
                                <div className={`card ${KartProducts[activeSession] && KartProducts[activeSession].length ? 'd-none' : ''}`} style={basePOS}>
                                    ...
                                    {['bx-drink', 'bx-coffee', 'bx-food-menu', 'bx-restaurant'].map((icon, i) => <i key={icon} className={`bx ${icon} ${activated !== i ? 'd-none' : ""}`} style={{ fontSize: 60 }} />)}
                                ...
                                {KartProducts[activeSession] && KartProducts[activeSession].map((item, index) => (
                                    ...
                                    >
                                        ...
                                        {order && (order.products ?? []).includes(item.id) && <small>{"Taste: "}<b>{item.taste}</b>{" Note:     (" + item.note + ")"}</small>}
                                        <div className={`d-flex`}>
                                            <span className={`quantity`}>
                                                {type === 'customer' && 'Qty:' + showRate(item.stock, item.unit)}
                                            </span>
                                            <span>{`${currency + ' ' + f(item.price)} / units`}</span>
                                        </div>
            ...
            {customerModalOpen && <Customer
                ...
                adding={addingCustomer}
                settings={settings}
                setAdding={setAddingCustomer}
                setData={setCustomers}
                addCustomer={addCustomer}
                handle={handleCustomer}
                select={selectCustomer}
                close={setCustomerModalOpen}
